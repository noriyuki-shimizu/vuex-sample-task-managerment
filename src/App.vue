    ...
    <ul>
      <li v-for="task in tasks" v-bind:key="task.id">
        <input type="checkbox" v-bind:checked="task.done"
          v-on:change="toggleTaskStates(task)">
        {{ task.name }}
        -
        <span v-for="id in task.lebelIds" v-bind:key="id">
          {{ getLabelText(id) }}
        </span>
      </li>
    </ul>
...
export default {
  ...
  methods: {
    ...
    // タスクの完了状態を更新する
    toggleTaskStates (task) {
      // `toggleTaskStates` ミューテーションをコミット
      this.$store.commit('toggleTaskStates', {
        id: task.id,
      })
    },
